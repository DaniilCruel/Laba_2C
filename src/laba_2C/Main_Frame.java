...
public class Main_Frame extends JFrame {
    ...
    public Double formula1(Double x, Double y, Double z)
    {
        ...
        return  pow(log((1+x)*(1+x))+cos(Math.PI*z*z*z),sin(y))+pow(exp(x*x)+cos(exp(z))+ pow(1/y,0.5),(1/x));
    }
    public Double formula2(Double x, Double y, Double z)
    {
        if (y ==-1 || x<=0)	{
            JOptionPane.showMessageDialog(Main_Frame.this,
                    "y должен != -1  x должен >1", "" +
                            "Ошибка ввода", JOptionPane.WARNING_MESSAGE);
            return 0.0;
        }
        if (log((1+y)*(1+y))+cos(Math.PI*x*x*x) <=0)	{
            JOptionPane.showMessageDialog(Main_Frame.this,
                    "log((1+y)*(1+y))+cos(Math.PI*x*x*x) Должен быть > 0", "" +
                            "Ошибка ввода", JOptionPane.WARNING_MESSAGE);
            return 0.0;
        }
        return pow(cos(Math.PI*x*x*x)+log((1+y)*(1+y)),1/4)*(exp(z*z)+cos(exp(y))+pow(1/x,1/2));
    }
